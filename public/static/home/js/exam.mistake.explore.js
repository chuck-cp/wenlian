layui.use(['jquery', 'layer'], function () {
    ...
    var getQuestionListUrl = function () {
        return `/exam/mistake/questions`;
    }

    var getQuestionInfoUrl = function (id, sn) {
        return `/exam/question/${id}/info?type=mistake&sn=${sn}`;
    }
    ...
    var getMistakeUrl = function (id) {
        return `/exam/question/${id}/mistake`;
    };
    ...
    var checkAnswer = function (questionId, questionModel, userAnswer) {
        $.ajax({
            method: 'POST',
    ...
    $('body').on('click', '.delete', function () {
        var id = $(this).data('id');
        layer.confirm('确定要移出错题本吗？', function (index) {
            $.ajax({
                method: 'POST',
                url: getMistakeUrl(id),
                success: function () {
                    disableQuestion(id);
                    layer.close(index);
                }
