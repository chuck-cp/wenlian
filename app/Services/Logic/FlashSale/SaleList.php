...
class SaleList extends LogicService
{
    ...
    protected function getDateSales($sales, $date)
    {
        ...
        foreach ($schedules as $schedule) {
            ...
            foreach ($sales as $sale) {
                ...
                $itemInfo = $this->handleItemInfo($sale->item_type, $sale->item_info);
            ...
            if ($status == FlashSaleModel::STATUS_ACTIVE) {
                $hasActiveStatus = true;
            }

            $result[] = [
                'hour' => sprintf('%02d:00', $hour),
                'selected' => $status == FlashSaleModel::STATUS_ACTIVE ? 1 : 0,
                'status' => $status,
                'items' => $items,
            ];
        }
    ...
    protected function getSaleStatus($date, $hour)
    {
        if (strtotime($date) - strtotime('today') > 0) {
            return FlashSaleModel::STATUS_PENDING;
        }

        $curHour = date('H');

        if ($curHour >= $hour + 2) {
            return FlashSaleModel::STATUS_EXPIRED;
        } elseif ($curHour >= $hour && $curHour < $hour + 2) {
            return FlashSaleModel::STATUS_ACTIVE;
        } else {
            return FlashSaleModel::STATUS_PENDING;
        }
    }
