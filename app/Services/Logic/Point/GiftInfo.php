...
class GiftInfo extends LogicService
{
    ...
    protected function handlePackagePointGift(PointGiftModel $gift)
    {
        $id = $gift->attrs['id'] ?? 0;

        if ($id == 0) return $gift;

        $package = $this->checkPackage($id);

        $gift->name = $package->title;
        $gift->cover = $package->cover;
        $gift->details = $package->summary;
    ...
    protected function handleExamPaperPointGift(PointGiftModel $gift)
    {
        $id = $gift->attrs['id'] ?? 0;
        ...
        $gift->details = $paper->details;

        return $gift;
    }

    protected function handleArticlePointGift(PointGiftModel $gift)
    {
        $id = $gift->attrs['id'] ?? 0;
