...
trait QuestionDataTrait
{
    ...
    {
        $status = $user->question_count > 100 ? QuestionModel::PUBLISH_APPROVED : QuestionModel::PUBLISH_PENDING;

        $settings = $this->getSettings('security.audit');

        if ($settings['question_enabled'] == 1) {
            $util = new ContentAuditUtil();
            $result = $util->auditHtml($content);
            if ($result == 0) {
                $status = QuestionModel::PUBLISH_APPROVED;
            } elseif ($result == 1) {
                $status = QuestionModel::PUBLISH_REJECTED;
            } else {
                $status = QuestionModel::PUBLISH_PENDING;
            }
        }
