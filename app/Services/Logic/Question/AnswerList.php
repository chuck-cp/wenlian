...
class AnswerList extends LogicService
{
    ...
    protected function handleAnswers($pager)
    {
        ...
        foreach ($answers as $answer) {
            ...
            $answer['content'] = $this->handleContent($answer['content']);
            $answer['images'] = json_decode($answer['images'], true);
    ...
    protected function getMeMappings($answers)
    {
        ...
        if ($user->id > 0) {
            ...
            $likedIds = $likeRepo->findUserLikedAnswerIds($user->id);
        }
        ...
        foreach ($answers as $answer) {
            $result[$answer['id']] = [
                'allow_comment' => $answer['closed'] == 0 ? 1 : 0,
