...
class Wxpay extends PayService
{
    ...
    public function notify()
    {
        ...
        $trade->channel_identity = $data->openid ?: '';
    ...
    public function find($tradeNo, $type = 'wap')
    {
        try {
            ...
            Log::error('Wxpay Find Order Exception', [
                'code' => $e->getCode(),
                'message' => $e->getMessage(),
            ]);
     ...
     * @return bool
     */
    public function withdraw(WithdrawModel $withdraw)
    {
        try {
            ...
            $response = $this->gateway->transfer([
                ...
                'amount' => 100 * $withdraw->trans_amount,
            ...
            Log::error('Wxpay Transfer Exception', [
                'code' => $e->getCode(),
                'message' => $e->getMessage(),
            ]);
