...
class Refund extends Service
{
    ...
    protected function previewExamPaperRefund(OrderModel $order)
    {
        $itemInfo = $order->item_info;

        $itemInfo['exam_paper']['cover'] = kg_cos_paper_cover_url($itemInfo['exam_paper']['cover']);
        ...
        if ($itemInfo['exam_paper']['refund_expiry_time'] > time()) {
            $refundRate = $this->getExamPaperRefundRate($order->item_id, $order->owner_id);
            $refundAmount = round(($order->amount - $serviceFee) * $refundRate, 2);
        }

        $itemInfo['exam_paper']['refund_rate'] = $refundRate;
        $itemInfo['exam_paper']['refund_amount'] = $refundAmount;
    ...
    protected function previewArticleRefund(OrderModel $order)
    {
    ...
    protected function getExamPaperRefundRate($paperId, $userId)
    {
        $paperUserRepo = new ExamPaperUserRepo();

        $paperUser = $paperUserRepo->findDebutPaperUser($paperId, $userId);

        if ($paperUser && $paperUser->start_time == 0) {
            return 1.00;
        }
