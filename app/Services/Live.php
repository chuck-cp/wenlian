...
class Live extends Service
{
    ...
    public function getStreamState($streamName, $appName = 'live')
    {
        try {
            ...
            $this->logger->debug('Describe LiveChat Stream State Request ' . $params);

            $response = $this->client->DescribeLiveStreamState($request);

            $this->logger->debug('Describe LiveChat Stream State Response ' . $response->toJsonString());

            $result = $response->StreamState;

        } catch (TencentCloudSDKException $e) {

            $this->logger->error('Describe LiveChat Stream State Exception ' . kg_json_encode([
                    'code' => $e->getErrorCode(),
    ...
    public function forbidStream($streamName, $appName = 'live', $reason = '')
    {
        try {
            ...
            $this->logger->debug('Forbid LiveChat Stream Request ' . $params);

            $response = $this->client->ForbidLiveStream($request);

            $this->logger->debug('Forbid LiveChat Stream Response ' . $response->toJsonString());

            $result = json_decode($response->toJsonString(), true);

        } catch (TencentCloudSDKException $e) {

            $this->logger->error('Forbid LiveChat Stream Exception ' . kg_json_encode([
                    'code' => $e->getErrorCode(),
                    'message' => $e->getMessage(),
                    'requestId' => $e->getRequestId(),
                ]));
    ...
    public function resumeStream($streamName, $appName = 'live')
    {
        try {
            ...
            $this->logger->debug('Resume LiveChat Stream Request ' . $params);

            $response = $this->client->ResumeLiveStream($request);

            $this->logger->debug('Resume LiveChat Stream Response ' . $response->toJsonString());

            $result = json_decode($response->toJsonString(), true);

        } catch (TencentCloudSDKException $e) {

            $this->logger->error('Resume LiveChat Stream Exception ' . kg_json_encode([
                    'code' => $e->getErrorCode(),
                    'message' => $e->getMessage(),
                    'requestId' => $e->getRequestId(),
                ]));
     ...
     * @return array|bool
     */
    public function createRecordTask($startTime, $endTime, $streamName, $appName = 'live')
    {
        try {

            $request = new CreateRecordTaskRequest();

            $params = json_encode([
                "TemplateId" => (int)$this->settings['push']['record_tpl_id'],
                'DomainName' => $this->settings['push']['domain'],
                'StreamName' => $streamName,
                'AppName' => $appName,
                "StartTime" => $startTime,
            ...
            $this->logger->debug('Create Record Task Request ' . $params);

            $this->client->setRegion('ap-guangzhou');

            $response = $this->client->CreateRecordTask($request);

            $this->logger->debug('Create Record Task Response ' . $response->toJsonString());

            $result = json_decode($response->toJsonString(), true);

        } catch (TencentCloudSDKException $e) {

            $this->logger->error('Create Record Task Exception ' . kg_json_encode([
                    'code' => $e->getErrorCode(),
                    'message' => $e->getMessage(),
                    'requestId' => $e->getRequestId(),
                ]));
        ...
        if ($transEnabled) {
            ...
            foreach ($formats as $format) {
                ...
                foreach (['od', 'hd', 'sd', 'fd'] as $rateName) {
                    ...
                    if (in_array($format, ['rtmp', 'webrtc'])) {
                        $realProtocol = $format;
                        $extension = '';
                    } else {
