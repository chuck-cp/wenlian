...
class ServerMonitorTask extends Task
{
    ...
    public function mainAction()
    {
        ...
        echo '------ start server monitor task ------' . PHP_EOL;
        ...
        if (count($items) > 0) {
            ...
            $serverId = $this->getConfig()->get('server_id');

            $host = parse_url($this->getSettings('site')['url'], PHP_URL_HOST);

            if (!empty($host)) {
                $items['host'] = sprintf('主机名称:%s', $host);
            }

            if (!empty($serverId)) {
                $items['node'] = sprintf('节点编号:%s', $serverId);
            }
        ...
        echo '------ end server monitor task ------' . PHP_EOL;
    }
