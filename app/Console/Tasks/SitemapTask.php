...
class SitemapTask extends Task
{
    ...
    protected function addExamPapers()
    {
        /**
         * @var Resultset|ExamPaperModel[] $papers
         */
        $papers = ExamPaperModel::query()
            ->where('published = 1')
            ->andWhere('deleted = 0')
            ->orderBy('id DESC')
            ->limit(500)
        ...
        foreach ($papers as $paper) {
            $loc = sprintf('%s/exam/paper/%s', $this->siteUrl, $paper->id);
            $this->sitemap->addItem($loc, 0.8);
        }
    }
    ...
    protected function addOthers()
    {
        ...
        $this->sitemap->addItem("{$this->siteUrl}/exam/paper/list", 0.6);
