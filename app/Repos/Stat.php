...
class Stat extends Repository
{
    ...
    public function countPendingRefunds()
    {
        return (int)RefundModel::count([
            'conditions' => 'status = :status: AND deleted = 0',
            'bind' => ['status' => RefundModel::STATUS_PENDING],
        ]);
    }
    ...
    public function countPendingInvoices()
    {
        return (int)InvoiceModel::count([
            'conditions' => 'status = :status: AND deleted = 0',
            'bind' => ['status' => InvoiceModel::STATUS_PENDING],
        ]);
    }

    public function countPendingPointGiftRedeems()
    {
        return (int)PointGiftRedeemModel::count([
            'conditions' => 'gift_type = :gift_type: AND status = :status:',
            'bind' => [
                'gift_type' => KgSaleModel::ITEM_GOODS,
                'status' => PointGiftRedeemModel::STATUS_PENDING,
            ],
        ]);
    }
    ...
    public function countPendingDanmus()
    {
        return (int)DanmuModel::count([
            'conditions' => 'published = :published: AND deleted = 0',
            'bind' => ['published' => DanmuModel::PUBLISH_PENDING],
        ]);
    }

    public function countReportedExamQuestions()
    {
        return (int)ExamQuestionModel::count([
