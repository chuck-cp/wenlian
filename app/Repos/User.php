...
class User extends Repository
{
    ...
    public function findGroups($userId)
    {
        return $this->modelsManager->createBuilder()
            ->columns('g.*')
            ->addFrom(GroupModel::class, 'g')
            ->join(GroupUserModel::class, 'g.id = gu.group_id', 'gu')
    ...
    public function countStudyPapers($userId)
    {
        return (int)ExamPaperUserModel::count([
            'conditions' => 'user_id = :user_id: AND debut = 1 AND deleted = 0',
            'bind' => ['user_id' => $userId],
        ]);
    }

    public function countStudyCourses($userId)
    {
    ...
    public function countStudyArticles($userId)
    {
