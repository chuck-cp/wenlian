...
class Course extends Repository
{
     ...
     * @return ResultsetInterface|Resultset|CourseModel[]
     */
    public function findShallowCourseByIds($ids)
    {
        return CourseModel::query()
     ...
     */
    public function findExamPapers($courseId)
    {
        return $this->modelsManager->createBuilder()
            ->columns('ep.*')
            ->addFrom(ExamPaperModel::class, 'ep')
            ->join(CourseExamPaperModel::class, 'ep.id = cep.paper_id', 'cep')
            ->where('cep.course_id = :course_id:', ['course_id' => $courseId])
            ->andWhere('ep.published = 1')
            ->andWhere('ep.deleted = 0')
            ->getQuery()->execute();
    }
    ...
    public function countExamPapers($courseId)
    {
        return $this->findExamPapers($courseId)->count();
    }
