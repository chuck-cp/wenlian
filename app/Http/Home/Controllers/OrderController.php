...
use App\Services\Logic\Order\CouponApply as OrderCouponApplyService;
use App\Services\Logic\Order\CouponList as OrderCouponListService;
...
class OrderController extends Controller
{
    ...
    {
        $itemId = $this->request->getQuery('item_id', ['trim', 'string']);
        $itemType = $this->request->getQuery('item_type', ['trim', 'string']);

        $service = new OrderConfirmService();

        $confirm = $service->handle($itemId, $itemType);

        $service = new OrderCouponListService();

        $coupons = $service->handle($itemId, $itemType);

        $this->seo->prependTitle('确认订单');

        $this->view->setVar('confirm', $confirm);
        $this->view->setVar('coupons', $coupons);
    }

    /**
     * @Post("/coupon", name="home.order.coupon")
     */
    public function couponAction()
    {
        $service = new OrderCouponApplyService();
