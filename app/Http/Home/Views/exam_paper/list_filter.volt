...
{% set exam_type_val = request.get('exam_type','trim','all') %}
    ...
    <div class="filter-group">
        ...
        <div class="content">
            {% for exam_type in exam_types %}
                {% set class = exam_type_val == exam_type.id ? 'layui-btn layui-btn-xs' : 'none' %}
                <a class="{{ class }}" href="{{ exam_type.url }}">{{ exam_type.name }}</a>
            {% endfor %}
        </div>
    </div>
    <div class="filter-group">
