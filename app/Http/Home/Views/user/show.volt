...
{% block content %}
    ...
    {% set show_tab_study_courses = user.study_course_count > 0 %}
    {% set show_tab_study_papers = user.study_paper_count > 0 %}
    {% set show_tab_study_articles = user.study_article_count > 0 %}
    {% set show_tab_questions = user.question_count > 0 %}
    {% set show_tab_answers = user.answer_count > 0 %}

    {% set study_courses_url = url({'for':'home.user.study_courses','id':user.id}) %}
    {% set study_papers_url = url({'for':'home.user.study_exam_papers','id':user.id}) %}
    {% set study_articles_url = url({'for':'home.user.study_articles','id':user.id}) %}
    ...
    <div class="tab-wrap">
        <div class="layui-tabs user-tab">
            <ul class="layui-tabs-header">
                ...
                {% if show_tab_study_papers %}
                    <li>在学试卷</li>
                {% endif %}
                {% if show_tab_study_articles %}
                    <li>在学专栏</li>
                {% endif %}
            ...
            <div class="layui-tabs-body">
                <div class="layui-tabs-item layui-show" id="tab-study-courses" data-url="{{ study_courses_url }}"></div>
                {% if show_tab_study_papers %}
                    <div class="layui-tabs-item" id="tab-study-papers" data-url="{{ study_papers_url }}"></div>
                {% endif %}
                {% if show_tab_study_articles %}
                    <div class="layui-tabs-item" id="tab-study-articles" data-url="{{ study_articles_url }}"></div>
                {% endif %}
                {% if show_tab_questions %}
