...
{% block content %}
    ...
    {%- macro item_info(confirm) %}
        ...
        {% elseif confirm.item_type == 4 %}
            ...
            <div class="order-item">
                <p>试卷名称：<span>{{ exam_paper.title }}</span></p>
                <p>退款期限：<span>{{ date('Y-m-d H:i:s',exam_paper.refund_expiry_time) }} {{ expiry_flag }}</span></p>
                <p>退款金额：<span class="price">{{ '￥%0.2f'|format(exam_paper.refund_amount) }}</span>退款比例：<span class="rate">{{ 100 * exam_paper.refund_rate }}%</span></p>
            </div>
        {% endif %}
    {%- endmacro %}
