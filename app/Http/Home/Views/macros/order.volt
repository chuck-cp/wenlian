{%- macro item_info(order) %}
    ...
    {% elseif order.item_type == 4 %}
        ...
        <div class="order-item">
            ...
            <!-- <p>
                <span>市场价格：<em class="price">{{ '￥%0.2f'|format(exam_paper.market_price) }}</em></span>
                <span>会员价格：<em class="price">{{ '￥%0.2f'|format(exam_paper.vip_price) }}</em></span>
            </p> -->
            <p>
                <span>学习期限：{{ date('Y-m-d',exam_paper.study_expiry_time) }}</span>
                <!-- <span>退款期限：{{ exam_paper.refund_expiry > 0 ? date('Y-m-d',exam_paper.refund_expiry_time) : '不支持' }}</span> -->
            </p>
        </div>
    {% elseif order.item_type == 5 %}
        {% set article = order.item_info.article %}
        <div class="order-item">
            <p>专栏标题：{{ article.title }}</p>
            <!-- <p>
                <span>市场价格：<em class="price">{{ '￥%0.2f'|format(article.market_price) }}</em></span>
                <span>会员价格：<em class="price">{{ '￥%0.2f'|format(article.vip_price) }}</em></span>
            </p> -->
        </div>
