{%- macro show_lesson_list(chapter) %}
    <ul class="lesson-list">
        {% for lesson in chapter.children %}
            ...
            {% elseif lesson.model == 5 %}
                <li class="lesson-item {{ priv }}" data-url="{{ url }}">{{ doc_lesson_info(lesson) }}</li>
            {% endif %}
        {% endfor %}
    </ul>
{%- endmacro %}
...
{%- macro live_lesson_info(lesson) %}
    <div class="left">
        ...
        {% if lesson.me.owned == 0 %}
            <span class="lock"><i class="iconfont icon-lock"></i></span>
        {% endif %}
        {% if lesson.attrs.stream.status == 'inactive' and lesson.attrs.playback.ready == 1 %}
...
{%- macro doc_lesson_info(lesson) %}
    <div class="left">
        <span class="model"><i class="iconfont icon-pdf"></i></span>
        <span class="title">{{ lesson.title }}</span>
        {% if lesson.me.duration > 0 %}
            <span class="study-time" title="学习时长：{{ lesson.me.duration|duration }}"><i class="layui-icon layui-icon-time"></i></span>
        {% endif %}
    ...
    <div class="right">
        <span class="size">{{ lesson.attrs.size|human_size }}</span>
    </div>
{%- endmacro %}
