...
use App\Services\Logic\Course\ExamPaperList as ExamPaperListService;
...
use App\Services\Logic\Course\Poster as CoursePosterService;
use App\Services\Logic\Course\ReviewList as ReviewListService;
use App\Services\Logic\Course\TagList as TagListService;
...
class CourseController extends Controller
{
    ...
    /**
     * @Get("/{id:[0-9]+}/last/study", name="api.course.last_study")
     */
    public function lastStudyAction($id)
    {
        ...
        $chapter = $service->handle($chapterId);

        return $this->jsonSuccess(['chapter' => $chapter]);
    }

    /**
     * @Get("/{id:[0-9]+}/poster", name="api.course.poster")
     */
    public function posterAction($id)
    {
        $service = new CoursePosterService();
    ...
    /**
     * @Get("/{id:[0-9]+}/exam/papers", name="api.course.exam_papers")
     */
    public function examPapersAction($id)
    {
        $service = new ExamPaperListService();

        $examPapers = $service->handle($id);

        return $this->jsonSuccess(['exam_papers' => $examPapers]);
    }
