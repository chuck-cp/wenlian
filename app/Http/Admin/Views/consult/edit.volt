...
{% block content %}
    ...
    <form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.consult.update','id':consult.id}) }}">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="radio" name="sticky" value="1" title="是" {% if consult.sticky == 1 %}checked="checked"{% endif %}>
                <input type="radio" name="sticky" value="0" title="否" {% if consult.sticky == 0 %}checked="checked"{% endif %}>
            </div>
        </div>
        <div class="layui-form-item">
