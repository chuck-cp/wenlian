...
{% block content %}
    ...
    <form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.digital_card.create'}) }}">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                {% for value in [1,3,6,12,24,36] %}
                    {% set checked = value == 1 ? 'checked="checked"' : '' %}
                    <input type="radio" name="expiry" value="{{ value }}" title="{{ value }}个月" {{ checked }}>
                {% endfor %}
            </div>
        </div>
        <div class="layui-form-item">
