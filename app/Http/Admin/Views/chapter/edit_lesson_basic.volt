<form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.chapter.update','id':chapter.id}) }}">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="radio" name="published" value="1" title="是" {% if chapter.published == 1 %}checked="checked"{% endif %}>
            <input type="radio" name="published" value="0" title="否" {% if chapter.published == 0 %}checked="checked"{% endif %}>
        </div>
    </div>
    <div class="layui-form-item">
