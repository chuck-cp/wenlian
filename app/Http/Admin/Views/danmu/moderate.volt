...
{% block content %}
    ...
    {% set owner_url = url({'for':'home.user.show','id':danmu.owner.id}) %}
    ...
    <div class="kg-mod-preview">
        <div class="meta">
            <span><a href="{{ owner_url }}" target="_blank">{{ danmu.owner.name }}</a></span>
            <span>{{ date('Y-m-d H:i:s',danmu.create_time) }}</span>
        </div>
        <div class="content">{{ danmu.text }}</div>
    </div>
    ...
    {% set moderate_url = url({'for':'admin.danmu.moderate','id':danmu.id}) %}
