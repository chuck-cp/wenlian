    ...
    <form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.setting.sms'}) }}">
        ...
        <table class="layui-table layui-form kg-table">
            ...
            <tr>
                <td>提现成功通知</td>
                <td>
                    <input type="radio" name="template[withdraw_finish][enabled]" value="1" title="是" {% if template.withdraw_finish.enabled == "1" %}checked="checked"{% endif %}>
                    <input type="radio" name="template[withdraw_finish][enabled]" value="0" title="否" {% if template.withdraw_finish.enabled == "0" %}checked="checked"{% endif %}>
                </td>
                <td><input class="layui-input" type="text" name="template[withdraw_finish][id]" value="{{ template.withdraw_finish.id }}" lay-verify="required"></td>
                <td><input id="tc-withdraw-finish" class="layui-input" type="text" value="提现成功，提现平台：{1}，提现金额：{2}元，到帐金额：{3}元，服务费用：{4}元" readonly="readonly"></td>
                <td><span class="kg-copy layui-btn" data-clipboard-target="#tc-withdraw-finish">复制</span></td>
            </tr>
            <tr>
                <td>分销成功通知</td>
                <td>
                    <input type="radio" name="template[dist_success][enabled]" value="1" title="是" {% if template.dist_success.enabled == "1" %}checked="checked"{% endif %}>
                    <input type="radio" name="template[dist_success][enabled]" value="0" title="否" {% if template.dist_success.enabled == "0" %}checked="checked"{% endif %}>
                </td>
                <td><input class="layui-input" type="text" name="template[dist_success][id]" value="{{ template.dist_success.id }}" lay-verify="required"></td>
                <td><input id="tc-dist-success" class="layui-input" type="text" value="分销成功，订单名称：{1}，订单金额：{2}元，奖励金额：{3}元" readonly="readonly"></td>
                <td><span class="kg-copy layui-btn" data-clipboard-target="#tc-dist-success">复制</span></td>
            </tr>
            <tr>
                <td>开票成功通知</td>
                <td>
                    <input type="radio" name="template[invoice_finish][enabled]" value="1" title="是" {% if template.invoice_finish.enabled == "1" %}checked="checked"{% endif %}>
                    <input type="radio" name="template[invoice_finish][enabled]" value="0" title="否" {% if template.invoice_finish.enabled == "0" %}checked="checked"{% endif %}>
                </td>
                <td><input class="layui-input" type="text" name="template[invoice_finish][id]" value="{{ template.invoice_finish.id }}" lay-verify="required"></td>
                <td><input id="tc-invoice-finish" class="layui-input" type="text" value="开票成功，发票抬头：{1}，发票金额：{2}元，发票类型：{3}" readonly="readonly"></td>
                <td><span class="kg-copy layui-btn" data-clipboard-target="#tc-invoice-finish">复制</span></td>
            </tr>
            <tr>
                <td>课程直播提醒</td>
                <td>
            ...
            <tr>
                <td>阅卷完成通知</td>
                <td>
                    <input type="radio" name="template[paper_grade_finish][enabled]" value="1" title="是" {% if template.paper_grade_finish.enabled == "1" %}checked="checked"{% endif %}>
                    <input type="radio" name="template[paper_grade_finish][enabled]" value="0" title="否" {% if template.paper_grade_finish.enabled == "0" %}checked="checked"{% endif %}>
                </td>
                <td><input class="layui-input" type="text" name="template[paper_grade_finish][id]" value="{{ template.paper_grade_finish.id }}" lay-verify="required"></td>
                <td><input id="tc-paper-grade-finish" class="layui-input" type="text" value="{1} 阅卷完成，试卷名称：{1}，试卷总分：{2}，用户得分：{3}，请登录系统查看详情。" readonly="readonly"></td>
                <td><span class="kg-copy layui-btn" data-clipboard-target="#tc-paper-grade-finish">复制</span></td>
            </tr>
            </tbody>
        </table>
