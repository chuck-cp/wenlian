...
<form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.setting.wechat_oa'}) }}">
    <table class="layui-table layui-form kg-table">
        ...
        <tr>
            <td>提现成功通知</td>
            <td>
                <input type="radio" name="notice_template[withdraw_finish][enabled]" value="1" title="是" {% if notice_template.withdraw_finish.enabled == "1" %}checked="checked"{% endif %}>
                <input type="radio" name="notice_template[withdraw_finish][enabled]" value="0" title="否" {% if notice_template.withdraw_finish.enabled == "0" %}checked="checked"{% endif %}>
            </td>
            <td><input class="layui-input" type="text" name="notice_template[withdraw_finish][id]" value="{{ notice_template.withdraw_finish.id }}" lay-verify="required"></td>
        </tr>
        <tr>
            <td>开票成功通知</td>
            <td>
                <input type="radio" name="notice_template[invoice_finish][enabled]" value="1" title="是" {% if notice_template.invoice_finish.enabled == "1" %}checked="checked"{% endif %}>
                <input type="radio" name="notice_template[invoice_finish][enabled]" value="0" title="否" {% if notice_template.invoice_finish.enabled == "0" %}checked="checked"{% endif %}>
            </td>
            <td><input class="layui-input" type="text" name="notice_template[invoice_finish][id]" value="{{ notice_template.invoice_finish.id }}" lay-verify="required"></td>
        </tr>
        <tr>
            <td>阅卷完成通知</td>
            <td>
                <input type="radio" name="notice_template[paper_grade_finish][enabled]" value="1" title="是" {% if notice_template.paper_grade_finish.enabled == "1" %}checked="checked"{% endif %}>
                <input type="radio" name="notice_template[paper_grade_finish][enabled]" value="0" title="否" {% if notice_template.paper_grade_finish.enabled == "0" %}checked="checked"{% endif %}>
            </td>
            <td><input class="layui-input" type="text" name="notice_template[paper_grade_finish][id]" value="{{ notice_template.paper_grade_finish.id }}" lay-verify="required"></td>
        </tr>
        </tbody>
    </table>
    <br>
