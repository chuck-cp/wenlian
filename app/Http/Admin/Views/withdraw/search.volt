...
{% block content %}
    ...
    {% if target == 'search' %}
        {% set action_url = url({'for':'admin.withdraw.list'}) %}
        {% set title = '搜索提现' %}
    {% else %}
        {% set action_url = url({'for':'admin.withdraw.export'}) %}
        {% set title = '导出提现' %}
    {% endif %}
    ...
    <form class="layui-form kg-form" method="GET" action="{{ action_url }}">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input class="layui-input" type="text" name="withdraw_id" placeholder="提现编号精确匹配">
            </div>
        </div>
        <div class="layui-form-item">
