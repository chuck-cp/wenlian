...
{% block content %}
    ...
    <table class="kg-table layui-table layui-form">
        ...
        {% for item in pager.items %}
            ...
            {% set orders_url = url({'for':'admin.coupon.orders','id':item.id}) %}
            <tr>
                <td>
                    <p>名称：{{ item.name }}</p>
                ...
                <td>
                    ...
                    {% else %}
                        <p>使用门槛：不限</p>
                    {% endif %}
                    <p>参与商品：{{ item_scope_info(item.item_id,item.item_type,item.item_info) }}</p>
                </td>
                <td>{{ attrs_info(item.type,item.attrs) }}</td>
                <td>{{ item.total_usage }} / {{ item.claim_count }} / {{ item.apply_count }}</td>
                <td>
                ...
                <td class="center">
                    <div class="kg-dropdown">
                        ...
                        <ul>
                            ...
                            {% endif %}
                            <hr>
                            <li><a href="{{ orders_url }}">使用记录</a></li>
                        </ul>
                    </div>
                </td>
            </tr>
