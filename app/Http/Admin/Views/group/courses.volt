...
{% block content %}
    ...
    {% set add_url = url({'for':'admin.group.add_course','id':group.id}) %}
    ...
    <table class="layui-table kg-table">
        ...
        {% for item in pager.items %}
            ...
            {% set delete_url = url({'for':'admin.group.delete_course','id':item.id}) %}
            <tr>
                <td>{{ item.course.id }}</td>
                <td><a href="{{ course_url }}" target="_blank">{{ item.course.title }}</a></td>
                <td>{{ '￥%0.2f'|format(item.course.market_price) }}</td>
