...
use App\Services\WeWorkNotice as WeWorkNoticeService;
...
class TestController extends Controller
{
    ...
    /**
     * @Post("/wework/robot", name="admin.test.wework_robot")
     */
    public function weworkRobotAction()
    {
        $noticeService = new WeWorkNoticeService();

        $result = $noticeService->test();

        if ($result) {
            return $this->jsonSuccess(['msg' => '发送消息成功，请到企业微信确认']);
        } else {
            return $this->jsonError(['msg' => '发送消息失败，请检查配置']);
        }
    }
