...
class IndexController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $licenseInfo = $indexService->getMyLicenseInfo();
    ...
    public function mainAction()
    {
        ...
        $bizReviewStat = $indexService->getBizReviewStat();

        $appInfo = $indexService->getAppInfo();
        $serverInfo = $indexService->getServerInfo();
        $licenseInfo = $indexService->getMyLicenseInfo();

        $this->checkLicense($licenseInfo, $globalStat);
        ...
        $this->view->setVar('biz_review_stat', $bizReviewStat);
        $this->view->setVar('app_info', $appInfo);
    ...
    protected function checkLicense($license, $stat)
    {
        if ($license['auth_type'] == 'expire_time') {
            if ($license['expire_time'] < time()) {
                return $this->response->redirect(['for' => 'admin.license']);
            }
        } else {
            if ($license['user_count'] < $stat['user_count']) {
                return $this->response->redirect(['for' => 'admin.license']);
            }
        }
    }
