...
class Package extends Service
{
    ...
    protected function syncSaleInfo(PackageModel $package)
    {
        if ($package->hasUpdated(['title', 'cover', 'market_price'])) {
            $sync = new FlashSaleSync();
            $sync->syncPackageInfo($package);
