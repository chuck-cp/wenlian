...
class Setting extends Service
{
    ...
    public function updateWechatMpSettings($section, $settings)
    {
        $this->updateSettings($section, $settings);
         ...
         */
        $paySettings = [
            'mini_app_id' => $settings['app_id'],
        ];

        $this->updateSettings('pay.wxpay', $paySettings);
    }

    public function updateMobileSettings($section, $tab, $settings)
    {
        if ($tab == 'index') {
            if (!isset($settings['index_module']['top'])) {
                $settings['index_module']['top'] = [];
            }
            if (!isset($settings['index_module']['nav'])) {
                $settings['index_module']['nav'] = [];
            }
            if (!isset($settings['index_module']['content'])) {
                $settings['index_module']['content'] = [];
            }
        }
