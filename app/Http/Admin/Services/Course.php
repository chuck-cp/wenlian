...
class Course extends Service
{
    ...
    public function updateCourse($id)
    {
        ...
        if (isset($post['xm_paper_ids'])) {
            $this->saveExamPapers($course, $post['xm_paper_ids']);
        }
    ...
    public function getXmExamPapers($id)
    {
        $courseRepo = new CourseRepo();
        ...
        foreach ($items as $item) {
            $result[] = [
                'name' => sprintf('%s - %s（¥%0.2f）', $item->id, $item->title, $item->market_price),
                'value' => $item->id,
                'selected' => in_array($item->id, $paperIds),
            ];
        }
    ...
    protected function saveExamPapers(CourseModel $course, $xmPaperIds)
    {
        $courseRepo = new CourseRepo();

        $courseExamPapers = $courseRepo->findExamPapers($course->id);

        $originPaperIds = [];

        if ($courseExamPapers->count() > 0) {
            foreach ($courseExamPapers as $paper) {
                $originPaperIds[] = $paper->id;
            }
        }

        $newPaperIds = $xmPaperIds ? explode(',', $xmPaperIds) : [];
        $addedPaperIds = array_diff($newPaperIds, $originPaperIds);

        if ($addedPaperIds) {
            foreach ($addedPaperIds as $paperId) {
                $courseExamPaper = new CourseExamPaperModel();
                $courseExamPaper->course_id = $course->id;
                $courseExamPaper->paper_id = $paperId;
                $courseExamPaper->create();
            }
        }

        $deletedPaperIds = array_diff($originPaperIds, $newPaperIds);

        if ($deletedPaperIds) {
            $courseExamPaperRepo = new CourseExamPaperRepo();
            foreach ($deletedPaperIds as $paperId) {
                $courseExamPaper = $courseExamPaperRepo->findCourseExamPaper($course->id, $paperId);
                if ($courseExamPaper) {
                    $courseExamPaper->delete();
                }
            }
        }

        $paperCount = $courseRepo->countExamPapers($course->id);
        $course->paper_count = $paperCount;
        $course->update();
    ...
    protected function syncSaleInfo(CourseModel $course)
    {
        if ($course->hasUpdated(['title', 'cover', 'market_price'])) {
            $sync = new FlashSaleSync();
            $sync->syncCourseInfo($course);
