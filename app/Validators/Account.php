...
use App\Services\LoginLimit as LoginLimitService;
...
class Account extends Validator
{
    ...
    public function checkUserLogin($name, $password)
    {
        ...
        $loginLimit = new LoginLimitService();

        if (!$loginLimit->checkFailedLogin($account->id)) {
            throw new BadRequestException('account.too_many_login_attempts');
        }

        if (!PasswordUtil::checkHash($password, $account->salt, $account->password)) {
            $loginLimit->incrFailedLogin($account->id);
            throw new BadRequestException('account.login_pwd_incorrect');
        }
